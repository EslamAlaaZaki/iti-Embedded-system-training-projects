_Display_Init:
;task1.c,23 :: 		void Display_Init() {
SUB	SP, SP, #4
STR	LR, [SP, #0]
;task1.c,25 :: 		TFT_Init_ILI9341_8bit(320, 240);
MOVS	R1, #240
MOVW	R0, #320
BL	_TFT_Init_ILI9341_8bit+0
;task1.c,26 :: 		Delay_ms(500);
MOVW	R7, #45225
MOVT	R7, #40
NOP
NOP
...
NOP
NOP
;task1.c,27 :: 		TFT_Fill_Screen(CL_WHITE);
MOVW	R0, #65535
BL	_TFT_Fill_Screen+0
;task1.c,28 :: 		TFT_Image(0, 40, WorldImage_bmp, 1);
MOVS	R3, #1
...
_Display_Cursor:
;task1.c,33 :: 		void Display_Cursor(signed int lat, signed int lon, char *lt_Str, char *lng_Str, unsigned int color) {
...
LDRH	R4, [SP, #20]
STRH	R4, [SP, #20]
;task1.c,39 :: 		latitude_y = ((160*(90 - lat))/180) + 40;
RSB	R5, R0, #90
SXTH	R5, R5
; lat end address is: 0 (R0)
MOVS	R4, #160
...
SXTH	R0, R4
;task1.c,40 :: 		longitude_x = (320* ((unsigned long)(lon + 180)))/360 - 10;
ADDW	R4, R1, #180
SXTH	R4, R4
; lon end address is: 4 (R1)
SXTH	R5, R4
...
MOV	R1, R4
;task1.c,43 :: 		TFT_Set_Pen(CL_RED, 1);
STR	R1, [SP, #4]
STR	R0, [SP, #8]
MOVS	R1, #1
MOVW	R0, #63488
...
LDR	R0, [SP, #8]
LDR	R1, [SP, #4]
;task1.c,44 :: 		TFT_H_Line(longitude_x - 2, longitude_x + 2, latitude_y );
ADDS	R5, R1, #2
SUBS	R4, R1, #2
STR	R1, [SP, #4]
STR	R0, [SP, #8]
...
LDR	R0, [SP, #8]
LDR	R1, [SP, #4]
;task1.c,45 :: 		TFT_V_Line( latitude_y - 2, latitude_y + 2, longitude_x );
ADDS	R5, R0, #2
SUBS	R4, R0, #2
...
SXTH	R0, R4
BL	_TFT_V_Line+0
;task1.c,47 :: 		TFT_Set_Font(&TFT_defaultFont, color, FO_HORIZONTAL);
MOVS	R2, #0
LDRH	R1, [SP, #20]
MOVW	R0, #lo_addr(_TFT_defaultFont+0)
MOVT	R0, #hi_addr(_TFT_defaultFont+0)
BL	_TFT_Set_Font+0
;task1.c,48 :: 		TFT_Write_Text(lt_Str, 160, 200);
MOVS	R2, #200
MOVS	R1, #160
LDR	R0, [SP, #12]
BL	_TFT_Write_Text+0
;task1.c,49 :: 		TFT_Write_Text(lng_Str, 160, 215);
MOVS	R2, #215
MOVS	R1, #160
LDR	R0, [SP, #16]
BL	_TFT_Write_Text+0
...
_GPS_messagePARSER:
;task1.c,56 :: 		void GPS_messagePARSER(void){
SUB	SP, SP, #8
STR	LR, [SP, #0]
;task1.c,57 :: 		int i=0;
;task1.c,58 :: 		for(i=0;i<768;i++)txt[i]= UART3_Read();
...
; i end address is: 12 (R3)
IT	AL
BAL	L_GPS_messagePARSER2
L_GPS_messagePARSER3:
;task1.c,59 :: 		string = strstr(txt,"$GPGGA");
...
STR	R0, [R1, #0]
;task1.c,60 :: 		if(string != 0) {                      // If txt array contains "$GPGLL" string we proceed...
CMP	R0, #0
IT	EQ
BEQ	L_GPS_messagePARSER5
;task1.c,61 :: 		if(string[43] == '1') {               // if "$GPGGA" 	 message have '1' sign in the 43-th
...
CMP	R0, #49
IT	NE
BNE	L_GPS_messagePARSER6
;task1.c,64 :: 		latitude = (string[18]-48)*10 + (string[19]-48);
...
STRH	R1, [R0, #0]
;task1.c,65 :: 		longitude = (string[30]-48)*100 + (string[31]-48)*10 + (string[32]-48);
MOV	R0, R3
LDR	R0, [R0, #0]
ADDS	R0, #30
LDRB	R0, [R0, #0]
...
STRH	R1, [R0, #0]
;task1.c,66 :: 		if(string[28] == 'S') {            // if the latitude is in the South direction it has minus sign
MOV	R0, R3
LDR	R0, [R0, #0]
ADDS	R0, #28
LDRB	R0, [R0, #0]
CMP	R0, #83
IT	NE
BNE	L_GPS_messagePARSER7
;task1.c,67 :: 		latitude = 0 - latitude;
...
STRH	R0, [R1, #0]
;task1.c,68 :: 		}
L_GPS_messagePARSER7:
;task1.c,69 :: 		if(string[41] == 'W') {            // if the longitude is in the West direction it has minus sign
MOVW	R0, #lo_addr(_string+0)
MOVT	R0, #hi_addr(_string+0)
LDR	R0, [R0, #0]
ADDS	R0, #41
...
CMP	R0, #87
IT	NE
BNE	L_GPS_messagePARSER8
;task1.c,70 :: 		longitude = 0 - longitude;
...
STRH	R0, [R1, #0]
;task1.c,71 :: 		}
L_GPS_messagePARSER8:
;task1.c,73 :: 		lat_str[0] = string[18];
...
STRB	R1, [R0, #0]
;task1.c,74 :: 		lat_str[1] = string[19];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #19
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;task1.c,77 :: 		lat_str[4] = string[21];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #21
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;task1.c,80 :: 		lat_str[7] = 0;
MOVS	R1, #0
MOVW	R0, #lo_addr(_lat_str+7)
MOVT	R0, #hi_addr(_lat_str+7)
STRB	R1, [R0, #0]
;task1.c,82 :: 		long_str[0] = string[30];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #30
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;task1.c,83 :: 		long_str[1] = string[31];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #31
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;task1.c,84 :: 		long_str[2] = string[32];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #32
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;task1.c,87 :: 		long_str[5] = string[34];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #34
LDRB	R1, [R0, #0]
...
_main:
;task1.c,95 :: 		void main() {
;task1.c,96 :: 		UART3_Init_Advanced(9600, _UART_8_BIT_DATA, _UART_NOPARITY, _UART_ONE_STOPBIT, &_GPIO_MODULE_USART3_PD89);   //Advanced Initialization
...
ADD	SP, SP, #4
;task1.c,97 :: 		Display_Init();
BL	_Display_Init+0
;task1.c,98 :: 		while(1)
L_main9:
;task1.c,100 :: 		GPS_messagePARSER(void);
BL	_GPS_messagePARSER+0
;task1.c,101 :: 		Display_Cursor(latitude, longitude,lat_str,long_str,CL_BLACK);
MOVW	R2, #0
MOVW	R0, #lo_addr(_longitude+0)
MOVT	R0, #hi_addr(_longitude+0)
LDRSH	R1, [R0, #0]
